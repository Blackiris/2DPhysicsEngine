...
void GameLogic::init_level() {
    ...
    auto circle_body1 = std::make_unique<RigidBody>(Transform2D(Vector2D(70, 100), 0), circle_shape, 10, 0.8);
    circle_body1->acc = Vector2D(0, 9.8);
    auto circle_body2 = std::make_unique<RigidBody>(Transform2D(Vector2D(300, 400), 0), circle_shape, 30, 0.8);
    ...
    auto square_body = std::make_unique<RigidBody>(Transform2D(Vector2D(500, 500), 0), square_shape, 50, 0.5);
    auto square_body2 = std::make_unique<RigidBody>(Transform2D(Vector2D(190, 300), 0), square_shape, 12, 0.8);
    ...
    auto circle_static_body = std::make_unique<StaticBody>(Transform2D(Vector2D(60, 300), 0), circle_shape);
    ...
    rigids.push_back(std::move(circle_static_body));
    rigids.push_back(std::move(convex_static_body2));
}
